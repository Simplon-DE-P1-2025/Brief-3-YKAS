...
# --- SÉLECTION DU MODE DE BASE DE DONNÉES ---
DB_MODE = os.getenv("DB_MODE", "duckdb")  # "duckdb" ou "postgresql"
...
if DB_MODE == "duckdb":
    # Mode DuckDB (pour déploiement cloud)
    try:
        from src.db_manager import db_manager
        from src.load_to_duckdb import duckdb_data_loader
        
        if not db_manager.is_initialized():
            with st.spinner("🔧 Initialisation de la base de données DuckDB..."):
            ...
            logging.info("✓ Base de données DuckDB déjà initialisée")
    except ImportError as e:
        logging.warning(f"Modules DuckDB non disponibles : {e}")
        st.info("Mode démo : Utilisation des fichiers Parquet uniquement")

elif DB_MODE == "postgresql":
    # Mode PostgreSQL (pour développement local avec Docker)
    from sqlalchemy import create_engine, text
    
    DB_URL = os.getenv("DATABASE_URL", "postgresql://admin:admin@localhost:5432/maritime")
...
def main():
    ...
    # === CRUD (PostgreSQL uniquement) ===
        ...
        with tab_create:
            ...
            with st.form("add"):
                c1, c2 = st.columns(2)
                next_id = int(df_smart['operation_id'].max() + 1) if not df_smart.empty else 1
                nid = c1.number_input("ID", value=next_id, step=1)
                ncross = c1.selectbox("CROSS", ["Etel", "Corsen", "Jobourg", "Gris-Nez", "La Garde", "Antilles-Guyane", "La Réunion"])
                nevt = c2.text_input("Événement", "SAR")
                ndate = c2.date_input("Date")
                
                if st.form_submit_button("Sauvegarder"):
                    sql = """INSERT INTO operations (operation_id, evenement, "cross", date_heure_reception_alerte) VALUES (:id, :evt, :cr, :dt)"""
                    ok, msg = run_query(sql, {"id": nid, "evt": nevt, "cr": ncross, "dt": ndate})
                    if ok:
                        st.success(f"Opération {nid} ajoutée !")
                        st.rerun()
                    else: 
                        st.error(msg)
        ...
        with tab_delete:
            ...
            if df_smart.empty:
                ...
                if data_del is not None:
                    st.divider()
                    st.warning(f"⚠️ Suppression de l'opération {del_id}")
                    c1, c2 = st.columns(2)
                    c1.text_input("CROSS", value=data_del['cross'], disabled=True)
                    c2.text_input("Événement", value=data_del['evenement'], disabled=True)
                    
                    if st.button("🔥 Confirmer Suppression", type="primary"):
                        ok, msg = run_query("DELETE FROM operations WHERE operation_id = :id", {"id": del_id})
                        if ok:
                            st.success(f"Opération {del_id} supprimée.")
                            st.rerun()
                        else:
                            st.error(msg)
